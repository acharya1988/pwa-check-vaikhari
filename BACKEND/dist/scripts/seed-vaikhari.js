...
async function main() {
    ...
    await Drift.updateOne({ userId, title: "Circadian Dinacharya & Agni Windows" }, {
        $setOnInsert: {
            userId,
    ...
    await Layer.updateOne({ userId, title: "On Hita-Ahita framing" }, {
        $setOnInsert: {
            userId,
    ...
    await LibraryItem.updateOne({ userId, refId: "ashtanga-hridaya" }, {
        $setOnInsert: {
            userId,
