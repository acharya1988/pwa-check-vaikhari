...
export async function listDrifts(userId: string, q?: string, status?: string) {
...
export async function createDrift(userId: string, body: any) {
  return DriftModel.create({ ...body, userId });
}

export async function getDrift(id: string) {
  return DriftModel.findById(id).lean();
}

export async function updateDrift(id: string, patch: any) {
  return DriftModel.findByIdAndUpdate(id, patch, { new: true }).lean();
}

export async function deleteDrift(id: string) {
