...
export default function NewStandaloneArticlePage() {
    ...
    const [currentView, setCurrentView] = useState<'publisher-select' | 'org-select'>('publisher-select');
    ...
    const handleFormCancel = () => {
        setIsFormOpen(false);
        // Delay resetting publisher to avoid UI flicker
        setTimeout(() => setSelectedPublisher(null), 300);
    }
    
    const handleBack = () => {
        if (currentView === 'org-select') {
            setCurrentView('publisher-select');
        }
    }
