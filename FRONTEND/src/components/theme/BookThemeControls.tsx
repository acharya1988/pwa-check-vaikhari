...
import type { ThemeStyles, StyleProperty, BookTheme } from '@/types';
...
export function BookThemeControls({ styles, defaultStyles, onStyleChange, onThemeApply }: { 
    styles: ThemeStyles, 
    defaultStyles: ThemeStyles, 
    onStyleChange: (element: keyof ThemeStyles, property: string, value: string) => void,
    onThemeApply: (theme: BookTheme) => void;
}) {
      ...
      <Tabs defaultValue="presets" className="w-full">
         ...
         <TabsContent value="presets">
            <div className="p-4 space-y-4">
                 {THEME_PRESETS.map((preset) => (
                    <div key={preset.themeName} className="flex items-center justify-between p-2 rounded-lg border bg-card hover:bg-muted">
                        <div>
                            <p className="font-semibold">{preset.themeName}</p>
                            <div className="flex items-center gap-2 mt-2">
                                <div className="w-5 h-5 rounded-full" style={{ background: preset.styles.h1?.color || 'transparent' }}></div>
                                <div className="w-5 h-5 rounded-full" style={{ background: preset.styles.paragraph?.color || 'transparent' }}></div>
                                <div className="w-5 h-5 rounded-full" style={{ background: preset.styles.sutra?.color || 'transparent' }}></div>
                            </div>
                        </div>
                        <Button size="sm" onClick={() => onThemeApply(preset)}>Apply</Button>
                    </div>
                ))}
            </div>
        </TabsContent>
        <TabsContent value="typography">
         ...
         <TabsContent value="elements">
             <div className="p-4">
                <Accordion type="multiple" className="w-full" defaultValue={['sutra']}>
