    ...
    useEffect(() => {
        if (state?.error) {
         ...
         if (state?.success) {
            ...
            // After success, sync the optimistic state with the parent's state, which should have been revalidated.
            if(state.action === 'removed') {
                setOptimisticIsBookmarked(false);
            } else if (state.action === 'added') {
                setOptimisticIsBookmarked(true);
            }
        }
