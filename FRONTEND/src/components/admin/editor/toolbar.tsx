  ...
  return (
    <div className="flex flex-wrap items-center gap-1 border-b border-input bg-background p-1" onMouseDown={(e) => e.preventDefault()}>
      ...
      <Separator orientation="vertical" className="h-8 mx-1" />
       ...
       <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="ghost" className="h-9 px-2.5 w-[120px] justify-between">
            <span className="truncate">{editor.getAttributes('textStyle').fontFamily || 'Font Family'}</span>
            <ChevronDown className="h-4 w-4" />
          </Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent>
          <DropdownMenuItem onClick={() => editor.chain().focus().unsetFontFamily().run()}>Default</DropdownMenuItem>
          <DropdownMenuItem style={{fontFamily: 'Inter'}} onClick={() => editor.chain().focus().setFontFamily('Inter').run()}>Inter (Body)</DropdownMenuItem>
          <DropdownMenuItem style={{fontFamily: 'Literata'}} onClick={() => editor.chain().focus().setFontFamily('Literata').run()}>Literata (Headline)</DropdownMenuItem>
          <DropdownMenuItem style={{fontFamily: 'Adishila'}} onClick={() => editor.chain().focus().setFontFamily('Adishila').run()}>Adishila (Sanskrit)</DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>

       <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="ghost" className="h-9 px-2.5 w-[100px] justify-between">
            <span className="truncate">{editor.getAttributes('textStyle').fontSize || 'Font Size'}</span>
            <ChevronDown className="h-4 w-4" />
          </Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent>
