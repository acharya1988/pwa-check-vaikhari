...
import { PlusCircle, ChevronDown, ChevronRight, FileText, Plus, Trash2, GripVertical, Edit, Library, Globe, Lock, Users, Palette, Annoyed, Mic, Waves } from 'lucide-react';
...
function AnnounceWithholdButtons({ bookId, isAnnounced, onUpdate }: { bookId: string, isAnnounced: boolean, onUpdate: () => void }) {
    ...
    const handleClick = (announce: boolean) => {
        const formData = new FormData();
        formData.append('bookId', bookId);
        formData.append('isAnnounced', String(announce));
        formAction(formData);
    }

    if (isAnnounced) {
        return (
            <Button variant="outline" size="sm" onClick={() => handleClick(false)}>
                Withhold
            </Button>
        );
    }
    ...
    return (
        <Button variant="default" size="sm" onClick={() => handleClick(true)}>
            Announce
        </Button>
    );
}
