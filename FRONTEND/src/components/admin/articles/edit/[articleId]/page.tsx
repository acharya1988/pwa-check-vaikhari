...
import type { StandaloneArticle, Quote, QuoteCategory, GlossaryCategory } from '@/types';
...
function EditStandaloneArticleClient({ articleId: articleIdFromProps }: { articleId: string }) {
    ...
    const fromPath = searchParams.get('from') || '/admin/articles';
    const fromLabel = fromPath.includes('drift') ? 'Drift Stream' : 'Articles';
    ...
    useEffect(() => {
        if (articleIdFromProps) {
            getStandaloneArticle(articleIdFromProps).then(data => {
                if (!data) {
                    notFound();
                } else {
                    setArticleData(data);
...
export default function EditStandaloneArticlePage() {
    const params = useParams();
    const articleId = params.articleId as string;
    return <EditStandaloneArticleClient articleId={articleId} />;
}
