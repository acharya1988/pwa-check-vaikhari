...
import type { TaggedContent, SuperAdminContent, Post } from '@/types';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Badge } from '@/components/ui/badge';
import { Search, BrainCircuit, Book, FileText, ChevronLeft, Sparkles, MessageSquare, Loader2 } from 'lucide-react';
import { ScrollArea } from '@/components/ui/scroll-area';
import { AnimatePresence, motion } from 'framer-motion';
import { cn } from '@/lib/utils';
import { stripHtml } from '@/services/service-utils';
import { runTagTrainingAction } from '@/actions/super-admin.actions';
...
function TrainingButton({ tag, items }: { tag: string; items: SuperAdminContent[] }) {
    const [state, formAction] = useActionState(runTagTrainingAction, null);
    const { toast } = useToast();
    const [isPending, startTransition] = React.useTransition();

    useEffect(() => {
        if (state?.success) {
            toast({ title: 'Training Started', description: state.message });
        }
        if (state?.error) {
            toast({ variant: 'destructive', title: 'Training Error', description: state.error });
        }
    }, [state, toast]);
    ...
    return (
        <Button onClick={handleTrain} disabled={isPending}>
            {isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Sparkles className="mr-2 h-4 w-4" />}
            Train on All {items.length} Items
        </Button>
    )
}
...
export function TagTrainerClient({ content }: { content: TaggedContent[] }) {
    ...
    const getPreviewText = (item: SuperAdminContent): string => {
        if (!item.content) return item.originalContentId;
        if (item.contentType === 'post') {
            const postContent = item.content?.content || '';
            const strippedContent = stripHtml(postContent);
            return strippedContent.substring(0, 100) + (strippedContent.length > 100 ? '...' : '');
        }
        return item.content.title || item.content.name || item.originalContentId;
    }
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="flex justify-end mb-4">
                             <TrainingButton tag={selectedTag.tag} items={selectedTag.items} />
                        </div>
                                        ...
                                        <div key={item.id} className="flex items-center gap-3 p-2 rounded hover:bg-muted">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <Link href={item.sourcePath} target="_blank" className="hover:underline">
                                                    <p className={cn("font-medium", item.contentType === 'post' ? 'line-clamp-2' : 'truncate')} title={previewText}>
                                                      {previewText}
                                                    </p>
                                                </Link>
                                    ...
                                    >
                                        <Card className="flex flex-col h-full hover:border-primary transition-colors">
                                             <CardHeader className="flex-grow">
                                                <CardTitle className="text-xl capitalize">#{group.tag}</CardTitle>
