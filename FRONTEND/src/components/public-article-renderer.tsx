...
import { Transliterate, useLanguage, SUPPORTED_LANGUAGES } from './transliteration-provider';
...
import { handleToggleGlow } from '@/app/articles/actions';
import { Popover, PopoverContent, PopoverTrigger } from '@/components/ui/popover';
import { cn } from '@/lib/utils';
import { ArticleMetadataBar } from './article-metadata-bar';
import { Languages, History, MoreHorizontal, StickyNote, MessageSquare, Layers, Lightbulb, Waypoints, Link2, GitBranch, Sparkles, Loader2, ChevronDown } from 'lucide-react';
        ...
        {isMounted && (
            ...
            {citation ? (
                <div className="flex flex-col gap-1">
                    ...
                    <span className="block font-headline text-primary"><Transliterate>{citation.sanskrit}</Transliterate></span>
  ...
  return (
    <TooltipProvider>
      <Tooltip>
        ...
        {isMounted && (
            <TooltipContent className="max-w-sm">
                <span className="font-bold font-devanagari block"><Transliterate>{term.term}</Transliterate></span>
...
function BlockTranslator({ sourceHtml, onTranslate, onReset }: { sourceHtml: string, onTranslate: (t: string) => void, onReset: () => void }) {
    ...
    const handleTranslate = async (lang: string) => {
        setIsLoading(true);
        try {
            const result = await translateText({ text: sourceHtml, targetLang: lang });
            ...
            <PopoverContent className="w-auto p-1">
                <div className="flex flex-col gap-1">
                    {SUPPORTED_LANGUAGES.map(lang => (
                        <Button key={lang.code} variant="ghost" size="sm" onClick={() => handleTranslate(lang.code)}>
...
function HtmlContentParser({ htmlString, isGlossaryMode, glossary, termsRegex, glossaryTermsMap, activeGlossaryColor }: {
    ...
    const memoizedContent = useMemo(() => {
        ...
        return <Transliterate>{parse(htmlString, options)}</Transliterate>;
...
function RenderedBlock({ block, isGlossaryMode, glossary, bookmarks, articleInfo, activeGlossaryColor, highlightTarget, isPreviewMode, onPostActivity, onDiscuss }: {
    block: ContentBlock;
    isGlossaryMode: boolean;
    glossary: GlossaryTerm[];
    ...
    return (
        <div id={block.id} data-block-type={block.type} className={cn("py-2 group relative rounded-md -mx-2 px-2", highlightTarget === block.id && 'highlight-flash')}>
            ...
            {!isCommentary && isSource && (
                 <div 
                    className="font-bold text-xl mb-4 text-foreground/80 font-headline" 
                >
...
export function ArticleHeader({ book, chapter, article, isBookmarked }: { book: BookContent, chapter: Chapter, article: Article, isBookmarked: boolean }) {
    return (
